...
PARAM$bo_lgb <- makeParamSet( 
         ...
         makeIntegerParam("num_leaves",       lower=    4L,   upper=  2048L), #Arrojó valores cercanos a 1024 en corrida anterior
         makeIntegerParam("min_data_in_leaf", lower=    1L,   upper= 50000L),
         makeNumericParam("lambda_l1",        lower=    0.0,   upper= 100.0),
         makeNumericParam("lambda_l2",        lower=    0.0,   upper= 200.0), #Se observó en corrida anterior que da valores cercanos a 100
         #makeNumericParam("min_gain_to_split",lower=    0.0,   upper= 15.0), #No fue necesario optimizar
        )
