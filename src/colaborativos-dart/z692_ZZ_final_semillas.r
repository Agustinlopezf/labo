...
#  y las tareas habituales de comparar el resultado de varias semillas en datos para los que se posee la clase

# SIEMPRE el final train se realiza SIN undersampling

# Acepta un vector de semillas y genera un modelo para cada semilla
# Acepta un vector de modelos_rank; se le puede especificar cuales modelos de la Bayesian Optimzation se quiere
#    modelos_rank es la posición en el ranking, por ganancia descendente  ( Atencion,NO es el campo iteracion_bayesiana )
#    por ejemplo, si  PARAM$modelos_rank  <- c( 1 ) entonces solo genera el mejor modelo
# En el caso que el daaset de future pose el campo clase_ternaria con todos los valores, se generan los graficos para las semillas en color gris y el promedio en color rojo
   # en el caso que se posea clase_completa con todos sus valores el dataset future, NO se generan los archivos para Kaggle
...
for( modelo_rank in  PARAM$modelos_rank )
{
  ...
  for( vsemilla  in PARAM$semillas )
  {
    ...
    if( future_con_clase )
    {
      ...
      tb_ganancias[ , paste0( "g", sem )  := tb_prediccion[ 1:PARAM$graficar$envios_hasta, ganancia_acum ] ]
      tb_ganancias[ , gan_sum := gan_sum + tb_prediccion[ 1:PARAM$graficar$envios_hasta, ganancia_acum ] ]
    }
  ...
  if( future_con_clase )
  {
    ...
    tb_ganancias[ , gan_suavizada := frollmean( x= gan_sum,
